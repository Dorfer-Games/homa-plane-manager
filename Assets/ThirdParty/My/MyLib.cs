...
public static class My : System.Object
{
	...
	public static string ScoreLerp(this TMPro.TextMeshProUGUI txt, float count, float t)
	{
		float startCount = int.Parse(txt.text);
		float result = Mathf.RoundToInt(Mathf.Lerp(startCount, count, t));
		if (startCount == result)
	...
	public static string ScoreLerp(this TMPro.TextMeshPro txt, float count, float t)
	{
		float startCount = int.Parse(txt.text);
		float result = Mathf.RoundToInt(Mathf.Lerp(startCount, count, t));
		if (startCount == result)
	...
	//convert float to hours : mins : secs : msecs
	public static string TimeMinSec(float Timer, bool msecs = false, bool hours = false)
	{
		string Result;
		int timersecs = (int)(Timer % 60);
		int timerhours = (int)((Timer - Timer % 3600) / 3600);
		int timermins = (int)((Timer - Timer % 60 - (hours ? timerhours * 3600 : 0)) / 60);

		float timermsecs = Timer % 1;
		string timerminsstring;
		string timersecsstring;

		timerminsstring = (timermins < 10f ? "0" : "") + timermins.ToString();
		timersecsstring = (timersecs < 10f ? "0" : "") + timersecs.ToString();
		Result = timerminsstring + ":" + timersecsstring;

		if (msecs) Result += ":" + (timermsecs * 100).ToString("F0");
		if (hours) Result = timerhours + ":" + Result;
	...
	public static void ParaMove(this Transform obj, Transform target, float time, System.Action onComplete, float width = 0, float height = 1, float depth = 0, DG.Tweening.Ease ease = DG.Tweening.Ease.Linear)
	{
		DOTween.Kill(obj);
		float Timer = 0;
	...
	public static void ParaMove(this Transform obj, Vector3 point, float time, System.Action onComplete, float width = 0, float height = 1, float depth = 0, DG.Tweening.Ease ease = DG.Tweening.Ease.Linear)
	{
		DOTween.Kill(obj);
		float Timer = 0;
		Vector3 StartPos = obj.position;
		DOTween.To(() => Timer, x => Timer = x, 1, time)
			.OnUpdate(() => obj.position = Parabola(StartPos, point, width, height, depth, Timer))
			.OnComplete(() =>
			{
				onComplete?.Invoke();
				obj.position = Parabola(StartPos, point, width, height, depth, Timer);
			}).SetEase(ease);
	}
	public static void ParaMoveLocal(this Transform obj, Vector3 point, float time, System.Action onComplete, float width = 0, float height = 1, float depth = 0, DG.Tweening.Ease ease = DG.Tweening.Ease.Linear)
	{
		DOTween.Kill(obj);
		float Timer = 0;
	...
	public static int PlusClamp(int target, int clamp)
	{
		if (target >= clamp)
			target = 0;
		else target++;
	...
	public static int MinusClamp(int target, int clamp)
	{
		if (target <= 0)
			target = clamp;
		else target--;
	...
	public static float PlusClamp(float target, float clamp)
	{
		if (target >= clamp)
			target = 0;
		else
	...
	public static float MinusClamp(float target, float clamp)
	{
		if (target <= 0)
			target = clamp;
		else target--;
	...
	public static void GrayUI(this Transform target, bool gray)
	{
		foreach (var item in target.GetComponentsInChildren<Image>())
			item.material.SetFloat("_EffectAmount", gray ? 1 : 0);
	}
	public static Vector3 RowColumn(int index, int rows, int columns, Vector3 offset)
	{
	...
	public static List<GameObject> SetActive(this List<GameObject> gameObjects, bool active)
	{
	...
	public static List<Transform> SetActive(this List<Transform> gameObjects, bool active)
	{
		foreach (var gameObj in gameObjects)
			gameObj.gameObject.SetActive(active);

		return gameObjects;
	}
	public static Color MaxColor(Color color)
	{
		float coeff = 1 / Mathf.Max(Mathf.Max(color.r, color.g), color.b);
		Color result = new Color(0, 0, 0, 1);
		result = color * coeff;
		return result;
	}
	public static Color RedGreenColor(float value)
	{
		Color result = Color.white;
		result.g = 1f - value * value;
		result.r = value * value;
		result = MaxColor(result);
		return result;
	}
	public static Color ColorLerp(Color startColor, Color targetColor, float percent, DG.Tweening.Ease ease = DG.Tweening.Ease.Linear)
    {
		Color result = Ease(percent, ease) * startColor / 2 + (1 - Ease(percent, ease)) * targetColor / 2;
		result = MaxColor(result);
		return result;
	}
	public static float Ease(float percent, DG.Tweening.Ease ease)
    {
		return DOVirtual.EasedValue(0, 1, percent, ease);
	}
}
